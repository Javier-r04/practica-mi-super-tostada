...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-[8rem] items-start gap-1.5 rounded-campo border border-[var(--border-subtle)] bg-blanco px-2.5 py-1.5 text-xs shadow-tarjeta",
        className,
      )}
    >
          ...
          .map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={cn(
                  "flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-tinta-500",
        ...
        .map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={cn(
                "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-tinta-500"
              )}
            >
