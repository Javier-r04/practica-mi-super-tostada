...
import type { ImportHandler } from "./import-handler";
...
export class ImportService {
  ...
  plantilla(tipo: string): {
    ...
    const clave = tipo.replace(/\.csv$/i, "");

    if (!(clave in PLANTILLAS_CSV)) {
    ...
    const importTipo = clave as ImportTipo;

    return {
      filename: `${importTipo}.csv`,
      csv: PLANTILLAS_CSV[importTipo],
    };
  }
  ...
  private async ejecutar(
    ...
    const validas = filas.filter((fila) => fila.ok).length;
    const invalidas = filas.filter((fila) => !fila.ok).length;
