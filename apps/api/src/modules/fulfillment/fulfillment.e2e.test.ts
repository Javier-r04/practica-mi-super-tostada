...
async function fixture(clock: Clock) {
  ...
  const [org] = await db
    .insert(organizacion)
    .values({ nombre: `org-e4-${crypto.randomUUID()}` })
...
describe.skipIf(!listo)("E4 operación diaria", () => {
  test("F-401 la hoja no existe hasta el cierre", async () => {
    ...
    try {
      await expect(f.hoja.obtener(f.orgId, "2026-08-21")).rejects.toMatchObject({
  ...
  test("F-401 cerrar a las 22:00 materializa v1, EN_PRODUCCION, evento + outbox + audit", async () => {
    ...
    try {
      ...
      const hoja = await f.hoja.obtener(f.orgId, "2026-08-21");
  ...
  test("F-402 sábado: DEMOCRACIA aparece bajo PLANTA", async () => {
    const f = await fixture(relojControlado(instanteGT("2026-08-21T16:00:00")));
    try {
      const { t16, tabasco } = await catalogoBasico(f);
      await f.pedidos.crearManual(
  ...
  test("F-405 a las 00:30 el manual extra cae en el día reabierto, no en el siguiente", async () => {
    ...
    try {
      ...
      expect(extra.fechaOperacion).toBe("2026-08-21");
      const v2 = await f.cierre.cerrar({}, f.actor);
      expect(v2.fechaOperacion).toBe("2026-08-21");
      expect(v2.version).toBe(2);
    } finally {
      await f.client.end({ timeout: 1 });
    }
  ...
  test("F-401 medianoche: cerrarSiToca cierra el 21, no el 22", async () => {
    ...
    try {
      ...
      expect(dia?.fechaOperacion).toBe("2026-08-21");
      expect(dia?.estado).toBe("CERRADO");
      const del22 = await f.db
        .select()
        .from(diaOperacion)
        .where(
          and(
            eq(diaOperacion.organizacionId, f.orgId),
            eq(diaOperacion.fechaOperacion, "2026-08-22"),
          ),
        );
      expect(del22).toHaveLength(0);
    } finally {
      await f.client.end({ timeout: 1 });
    }
  });
